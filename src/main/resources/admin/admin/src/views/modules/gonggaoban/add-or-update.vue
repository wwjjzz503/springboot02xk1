	...
	<div class="addEdit-block" :style='{"padding":"30px"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' class="input" v-if="type!='info'"  label="经理账号" prop="jinglizhanghao">
					<el-input v-model="ruleForm.jinglizhanghao" placeholder="经理账号" clearable  :readonly="ro.jinglizhanghao"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' v-else class="input" label="经理账号" prop="jinglizhanghao">
				...
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' v-if="type!='info'"  label="内容" prop="neirong">
					<editor 
				...
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' v-else-if="ruleForm.neirong" label="内容" prop="neirong">
...
export default {
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.neirong = this.ruleForm.neirong.replace(reg,'../../../springboot02xk1/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `gonggaoban/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.gonggaobanCrossAddOrUpdateFlag = false;
						 this.parent.search();

	...
	<div class="addEdit-block" :style='{"padding":"30px"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' class="date" v-if="type!='info'" label="留言时间" prop="liuyanshijian">
					<el-date-picker
				...
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' class="input" v-else-if="ruleForm.liuyanshijian" label="留言时间" prop="liuyanshijian">
					<el-input v-model="ruleForm.liuyanshijian" placeholder="留言时间" readonly></el-input>
				</el-form-item>
			</template>
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' v-if="type!='info'"  label="留言内容" prop="liuyanneirong">
					<editor 
				...
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' v-else-if="ruleForm.liuyanneirong" label="留言内容" prop="liuyanneirong">
                    <span :style='{"fontSize":"14px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.liuyanneirong"></span>
                </el-form-item>
...
export default {
	...
	created() {
		this.ruleForm.liuyanshijian = this.getCurDate()
	},
	methods: {
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.liuyanneirong = this.ruleForm.liuyanneirong.replace(reg,'../../../springboot02xk1/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `liuyanban/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.liuyanbanCrossAddOrUpdateFlag = false;
						 this.parent.search();
